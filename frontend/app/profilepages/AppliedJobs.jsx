...
export default function AppliedJobs() {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      if (session?.user?.id) {
        try {
          const response = await fetch(`http://localhost:5000/api/applications/user/${session.user.id}`);
          if (response.ok) {
        ...
        {applications.length === 0 ? (
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {applications.map((app) => (
              ...
              >
                <img
                  src={app.jobId?.companyLogo ? `http://localhost:5000${app.jobId.companyLogo}` : "/images/oracle.jpg"}
                  alt="Job"
